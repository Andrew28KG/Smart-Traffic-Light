...
class VideoStreamProcessor:
    def __init__(self, stream_url, model_path='my_model.pt', confidence=0.25, road_section_id=1):
        """
        ...
        # Initialize tracker for vehicle tracking
        # Lower max_age for less persistence after leaving view, min_hits=1 to start tracking immediately,
        # higher IOU threshold to maintain identity better
        ...
        while self.is_running:
            try:
                ...
                if not self.frame_queue.empty():
                    ...
                    # Vehicle detection preparation for tracking
                    dets = []
                    class_names = []
                    ...
                    if len(results) > 0 and len(results[0].boxes) > 0:
                        for box in results[0].boxes.data:
                            ...
                            if class_name in self.vehicle_classes:
                                # Add detection to tracker input
                                dets.append([float(x1), float(y1), float(x2), float(y2), float(conf), int(cls)])
                                class_names.append(class_name)
                    
                    # Update tracker with current detections - always update even with empty detections
                    dets_array = np.array(dets) if dets else np.empty((0, 6))
                    tracked_vehicles = self.tracker.update(dets_array, class_names)
                    
                    # Count vehicles based on tracking results
                    current_vehicle_counts = defaultdict(int)
                    # Count unique vehicles by class
                    vehicle_classes_seen = set()
                    for _, _, class_name in tracked_vehicles:
                        if class_name in self.vehicle_classes:
                            current_vehicle_counts[class_name] += 1
                            vehicle_classes_seen.add(class_name)
                    
                    current_total_vehicles = len(tracked_vehicles)
                    
                    # Create frame_vehicles from tracked vehicles
                    frame_vehicles = []
                    for bbox, track_id, class_name in tracked_vehicles:
                        x1, y1, x2, y2 = bbox
                        # Pass tracked vehicle box with track_id for display
        ...
        while self.is_running:
            try:
                if not self.result_queue.empty():
                    ...
                    # Draw tracking boxes and information with IDs
                    if vehicles and len(vehicles) > 0:
                        # Create a colormap for different tracking IDs
                        colors = [(0, 255, 0), (0, 165, 255), (0, 0, 255), (255, 0, 0), 
                                 (255, 255, 0), (0, 255, 255), (255, 0, 255)]
                         
                        for vehicle_class, (x1, y1, x2, y2, track_id) in vehicles:
                            ...
                            # Get color based on track ID for consistent coloring
                            ...
                            # Add track ID text at the top of the box
                            label = f"{vehicle_class}-{int(track_id)}"
